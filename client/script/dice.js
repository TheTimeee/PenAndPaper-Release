class Dice
{
    ...
    static ReadConfig()
    {
        ...
        {
            let aRect = [(screen.width / 2) - (cWindowAttributes.Dice.get("width") / 2), (screen.height / 2) - (cWindowAttributes.Dice.get("height") / 2), cWindowAttributes.Dice.get("width"), cWindowAttributes.Dice.get("height")];
        ...
        //Connection State
        let bConnected = PARENT.GetSessionField("dice_connected");
        let sIP = PARENT.GetSessionField("dice_ip");
        let sPort = PARENT.GetSessionField("dice_port");
        if (bConnected == true && sIP.length > 0 && sPort.length > 0)
        {
    ...
    static async #Connect()
    {
        ...
        try
        {
            ...
            PARENT.SetSessionField("dice_connected", 1);
            PARENT.SetSessionField("dice_ip", Dice.#_sIP);
            PARENT.SetSessionField("dice_port", Dice.#_sPort);
        ...
        catch (error)
        {
            ...
            PARENT.SetSessionField("dice_connected", 0);
            PARENT.SetSessionField("dice_ip", "");
            PARENT.SetSessionField("dice_port", "0");
    ...
    static #RollLocal()
    {
        ...
        sResult += "-----------------------------\n";
        sResult += LOCALIZED_MAP.get("IDD_ROLL_AMOUNT") + ": " + oContainer[2] + "\n";
    ...
    static async #RollServer()
    {
        ...
        try
        {
            ...
            if (oResponse.status === 200)
            {
                ...
                sResult = sResult.replace(new RegExp("IDD_ROLL_AMOUNT*"), LOCALIZED_MAP.get("IDD_ROLL_AMOUNT"));
